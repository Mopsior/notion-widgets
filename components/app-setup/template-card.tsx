...
export const TemplateCard = ({ name, description, icon, code, index }: { name: string, description: string, icon: string, code: string, index: number }) => {
    return (
        <Link href={`/app/new/${code}`}>
            <div className={`w-[400px] h-fit border rounded-notion shadow-notio hover:bg-muted/30 animate-slide-up transition-transform`} style={{ animationDelay: `${index * 0.2}s` }}>
